...
mwExtensionUnderTest ?= HitCounters
...
phpIni ?= $(shell echo ${PWD}/${mwCiPath}/php-settings.ini)
mwCiPath ?= ci
dockerCli ?= podman
...
startWiki: ${lsPath}
	...
	test -n "$${cid}"																			||	\
		cid=`${dockerCli} run -d -p "${mwWebPort}:80" --name=${mwContainer}							\
				-v "${PWD}/${mwDbPath}:${MW_DATA_DIR}"												\
				-v "${PWD}/${mwVendor}:${contPath}/vendor"											\
				-v "${PWD}:${contPath}/extensions/${mwExtensionUnderTest}"							\
				-v "${PWD}/${lsPath}:${contPath}/LocalSettings.php"									\
...
${lsPath}: ${phpIni} MW_DB_PATH MW_VENDOR
	test -f "$@"																			||	(	\
		...
		lsc=`${dockerCli} run -d																	\
			...
			-v "${PWD}/${mwVendor}:${contPath}/vendor"  ${mwImgVersion}`						&&	\
...
devVendor: MW_VENDOR MW_APT_CACHE MW_COMPOSER composer
	test -z "`find ${mwVendor}/composer/autoload_static.php -mmin +1440 -print`"			||	(	\
		...
		vsc=`${dockerCli} run -d																	\
			-v "${PWD}/${mwVendor}:${contPath}/vendor"												\
			-v "${PWD}/${mwDotComposer}:/root/.cache/composer"										\
			-v "${PWD}/${mwAptPath}:/var/cache/apt" ${mwImgVersion}`							&&	\
		...
		echo ok																					&&	\
		echo -n "${indent}- Running composer update ... "										&&	\
...
MW_VENDOR:
	test -d "${mwVendor}"																	||	(	\
		...
		vid=`${dockerCli} run -d ${mwImgVersion}`												&&	\
